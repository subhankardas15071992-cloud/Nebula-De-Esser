...
// ─────────────────────────────────────────────────────────────────────────────
// Nebula DeEsser v2 — Spectrum Analyzer
...
impl SpectrumAnalyzer {
    ...
    fn compute_fft(&mut self) {
        // Linearize ring buffer into complex buffer with Hann window applied
        for i in 0..FFT_SIZE {
            let idx = (self.write_pos + i) % FFT_SIZE;
            self.fft_output[i] =
                rustfft::num_complex::Complex::new(self.ring_buffer[idx] * self.window[i], 0.0);
        }
        ...
        // Scale: divide by FFT_SIZE, multiply by 2 for single-sided spectrum (except DC/Nyquist)
