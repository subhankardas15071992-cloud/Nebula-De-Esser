    ...
    fn default() -> Self {
        Self {
            ...
            threshold: FloatParam::new("Threshold", -20.0, FloatRange::Linear { min: -60.0, max: 0.0 })
                .with_unit(" dB")
                .with_step_size(0.1),
            max_reduction: FloatParam::new("Max Reduction", 12.0, FloatRange::Linear { min: 0.0, max: 40.0 })
                .with_unit(" dB")
                .with_step_size(0.1),
            min_freq: FloatParam::new("Min Frequency", 4000.0, FloatRange::Skewed { min: 1000.0, max: 16000.0, factor: FloatRange::skew_factor(-1.5) })
                .with_unit(" Hz")
                .with_step_size(1.0),
            max_freq: FloatParam::new("Max Frequency", 12000.0, FloatRange::Skewed { min: 1000.0, max: 20000.0, factor: FloatRange::skew_factor(-1.5) })
                .with_unit(" Hz")
                .with_step_size(1.0),
            ...
            lookahead_ms: FloatParam::new("Lookahead", 2.0, FloatRange::Linear { min: 0.0, max: 20.0 })
                .with_unit(" ms")
                .with_step_size(0.1),
            trigger_hear: FloatParam::new("Trigger Hear", 0.0, FloatRange::Linear { min: 0.0, max: 1.0 }),
            stereo_link: FloatParam::new("Stereo Link", 1.0, FloatRange::Linear { min: 0.0, max: 1.0 })
                .with_step_size(0.01),
            ...
            input_level: FloatParam::new("Input Level", 0.0, FloatRange::Linear { min: -60.0, max: 12.0 })
                .with_unit(" dB")
                .with_step_size(0.1),
            input_pan: FloatParam::new("Input Pan", 0.0, FloatRange::Linear { min: -1.0, max: 1.0 })
                .with_step_size(0.01),
            output_level: FloatParam::new("Output Level", 0.0, FloatRange::Linear { min: -60.0, max: 12.0 })
                .with_unit(" dB")
                .with_step_size(0.1),
            output_pan: FloatParam::new("Output Pan", 0.0, FloatRange::Linear { min: -1.0, max: 1.0 })
                .with_step_size(0.01),
            bypass: FloatParam::new("Bypass", 0.0, FloatRange::Linear { min: 0.0, max: 1.0 }),
            oversampling: FloatParam::new("Oversampling", 0.0, FloatRange::Linear { min: 0.0, max: 4.0 })
                .with_step_size(1.0),
...
struct NebulaDeEsser {
    ...
    prev_in_l: f64,
    prev_in_r: f64,
    out_l_buffer: Vec<f32>,
    out_r_buffer: Vec<f32>,
}
...
impl Plugin for NebulaDeEsser {
    ...
    fn process(&mut self, buffer: &mut Buffer, aux: &mut AuxiliaryBuffers, ctx: &mut impl ProcessContext<Self>) -> ProcessStatus {
        ...
        for s in 0..n {
            ...
            out_l_slice[s] = ol as f32;
            out_r_slice[s] = or_ as f32;
        ...
        if !out_slice.is_empty() {
            for (s, sample) in out_slice[0].iter_mut().enumerate() { *sample = out_l_slice[s]; }
        }
        if out_slice.len() > 1 {
            for (s, sample) in out_slice[1].iter_mut().enumerate() { *sample = out_r_slice[s]; }
        }
