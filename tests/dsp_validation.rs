...
mod dsp_validation {
    ...
    fn test_null_signal() {
        ...
        assert!(rms.abs() < 1e-10, "Silence should have near-zero RMS: {}", rms);
    }
    ...
    fn test_impulse_response() {
        ...
        assert_eq!(non_zero_count, 1, "Impulse should have exactly one non-zero sample");
    }
    ...
    fn test_oversampling_benefits() {
        ...
        assert!(test_freq < oversampled_nyquist,
    ...
    fn test_denormal_handling() {
        ...
        let flushed = if denormal.abs() < 1e-38 { 0.0 } else { denormal };
    ...
    fn test_buffer_size_robustness() {
        ...
        for &size in &buffer_sizes {
            ...
            assert_eq!(test_signal.len(), size, "Should generate correct buffer size");
    ...
    fn test_parameter_smoothing_anti_zipper() {
        ...
        for i in 0..=steps {
            ...
            if i > 0 {
                ...
                assert!(delta <= 1.0 / steps as f64 + 1e-10,
                       "Parameter change too abrupt at step {}: delta={}", i, delta);
            }
            previous = current;
        }
    }
    ...
    fn test_latency_consistency() {
        ...
        for &lookahead_ms in &lookahead_options {
            ...
            assert!((expected_ms - lookahead_ms).abs() < 1.0,
                   "Latency calculation mismatch: {}ms vs {}ms", expected_ms, lookahead_ms);
        }
    }
    ...
    fn test_spectral_balance() {
        ...
        let tolerance = 0.01;
        assert!((low_rms - expected_rms).abs() < tolerance, "Low freq RMS incorrect");
        assert!((mid_rms - expected_rms).abs() < tolerance, "Mid freq RMS incorrect");
        assert!((high_rms - expected_rms).abs() < tolerance, "High freq RMS incorrect");
    }
    ...
    fn test_transient_preservation() {
        ...
        assert!(energy > 0.9, "Impulse energy should be concentrated: {}", energy);
    }
