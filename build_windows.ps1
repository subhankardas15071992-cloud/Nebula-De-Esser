...
# ── Build ────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "[*] Building RELEASE binary (Windows VST3 requires release mode)..."
# CRITICAL: Force panic=abort and release optimizations for Windows stability
$env:RUSTFLAGS = "-C target-cpu=x86-64-v2 -C panic=abort -C opt-level=3 -C codegen-units=1"
...
cargo nih-plug bundle $PluginName --release --target x86_64-pc-windows-msvc
...
if ($clapFile) {
 ...
 Write-Host " $env:COMMONPROGRAMFILESCLAP" -ForegroundColor White
 Write-Host " — or —"
 Write-Host " $env:LOCALAPPDATAProgramsCommonCLAP" -ForegroundColor White
 Write-Host ""

 $installDest = "$env:COMMONPROGRAMFILESCLAP"
